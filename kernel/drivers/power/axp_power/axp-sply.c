...
static void  usb_check_delaywork_func(struct work_struct *work)
{
	uint8_t val;
	struct delayed_work *delaywork = container_of(work, struct delayed_work, work);
	struct axp_charger *charger = container_of(delaywork, struct axp_charger, delaywork);
	printk("[AXP]usb_check_delaywork_func\n");
	pc_or_charger = dwc_connect_status();
	otg_phy_suspend = dwc_vbus_status();
	if (pc_or_charger == 4)
	{
		axp_usb_curlim(0);
		printk("[AXP]set usb current set no limit\n");
		axp_read(charger->master,0x30,&val);
		printk("[AXP]ac or usb plug out,set REG30H =0x%02x\n",val);
        ...
        void *data)
{
	...
	if(event & (AXP_IRQ_ACIN|AXP_IRQ_ACRE|AXP_IRQ_USBIN|AXP_IRQ_USBRE|AXP_IRQ_ACOV|AXP_IRQ_USBOV|AXP_IRQ_CHAOV
				|AXP_IRQ_CHAST|AXP_IRQ_TEMOV|AXP_IRQ_TEMLO)) {
		if(event & (AXP_IRQ_ACIN|AXP_IRQ_USBIN))//ac or usb plug in
		{
			schedule_delayed_work(&charger->delaywork, msecs_to_jiffies(10000));
		}
		else if (event & (AXP_IRQ_ACRE|AXP_IRQ_USBRE))//ac or usb plug out
		{
			axp_usb_curlim(100);
			axp_read(charger->master,0x30,&val);
			printk("[AXP]ac or usb plug out,set usb current set 500mA,0x%02x\n",val);
		}
...
static int axp_battery_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&charger->delaywork, usb_check_delaywork_func);
	return ret;
