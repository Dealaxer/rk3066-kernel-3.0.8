...
static ssize_t sn_read(struct class *cls, struct class_attribute *attr, char *_buf)
#else
static ssize_t sn_read(struct class *cls, char *_buf)
#endif
{
    int count = 0;
    char sn_buffer[512];
    ...
    GetSNSectorInfo(sn_buffer);
    strncpy(temp, sn_buffer , 2);\
    sn_len  = (int)temp[1]*10 + (int)temp[0];
    strncpy(temp, sn_buffer + 2, sn_len);
    temp[sn_len] = 0;
    count = sprintf(_buf, "%s", temp);
//    count = sprintf(_buf,"%s", "Hello i love you sn number\n");
    printk("Current sn number you got it.\n");


}

#ifdef CONFIG_SND_SOC_RT5633
extern int codec_read_hp_state();

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 37))
static ssize_t codec_read_state(struct class *cls, struct class_attribute *attr, char *_buf)
#else
static ssize_t codec_read_state(struct class *cls, char *_buf)
#endif
{
    int  is_spk;
    int  count = 0;
    is_spk = codec_read_hp_state();
    count = sprintf(_buf,"%d",is_spk);
    printk("Current codec hp state is (%s)\n",_buf); 
...
static CLASS_ATTR(sn, 0664, sn_read, NULL);
#ifdef CONFIG_SND_SOC_RT5633
static CLASS_ATTR(hp_state,0664,codec_read_state, NULL);
#endif
...
int rkwifi_sysif_init(void)
{
    ...
    ret =  class_create_file(rkwifi_class, &class_attr_sn);
#ifdef CONFIG_SND_SOC_RT5633
    ret =  class_create_file(rkwifi_class, &class_attr_hp_state);
#endif
...
void rkwifi_sysif_exit(void)
{
    ...
    class_remove_file(rkwifi_class, &class_attr_sn);
#ifdef CONFIG_SND_SOC_RT5633
    class_remove_file(rkwifi_class, &class_attr_hp_state);
#endif
