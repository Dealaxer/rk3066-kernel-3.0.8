...
#define SENSOR_NAME_01 STR(CONFIG_SENSOR_01)			
...
static int rk_sensor_power(struct device *dev, int on);
    ...
    || (CONFIG_SENSOR_RESET_PIN_01 != INVALID_GPIO) || (CONFIG_SENSOR_RESET_PIN_11 != INVALID_GPIO) /*\
    || (CONFIG_SENSOR_RESET_PIN_03 != INVALID_GPIO) || (CONFIG_SENSOR_RESET_PIN_13 != INVALID_GPIO) \
    || (CONFIG_SENSOR_RESET_PIN_04 != INVALID_GPIO) || (CONFIG_SENSOR_RESET_PIN_14 != INVALID_GPIO) \
    || (CONFIG_SENSOR_RESET_PIN_05 != INVALID_GPIO) || (CONFIG_SENSOR_RESET_PIN_15 != INVALID_GPIO) \
    || (CONFIG_SENSOR_RESET_PIN_06 != INVALID_GPIO) || (CONFIG_SENSOR_RESET_PIN_16 != INVALID_GPIO) \
    || (CONFIG_SENSOR_RESET_PIN_07 != INVALID_GPIO) || (CONFIG_SENSOR_RESET_PIN_17 != INVALID_GPIO) \
    || (CONFIG_SENSOR_RESET_PIN_08 != INVALID_GPIO) || (CONFIG_SENSOR_RESET_PIN_18 != INVALID_GPIO) \
    || (CONFIG_SENSOR_RESET_PIN_09 != INVALID_GPIO) || (CONFIG_SENSOR_RESET_PIN_19 != INVALID_GPIO) \
    || (CONFIG_SENSOR_RESET_PIN_02 != INVALID_GPIO) || (CONFIG_SENSOR_RESET_PIN_12 != INVALID_GPIO)
*/
    ...
    if (camera_flash != INVALID_GPIO) {
		if (camera_io_init & RK29_CAM_FLASHACTIVE_MASK) {
            switch (on)
            {
                ...
                default:
                {
                    dprintk("%s Flash command(%d) is invalidate", res->dev_name,on);
                    break;
                }
            }
		} else {
			ret = RK29_CAM_EIO_REQUESTFAIL;
			dprintk("%s FlashPin=%d request failed!", res->dev_name,camera_flash);
		}
    } else {
		ret = RK29_CAM_EIO_INVALID;
    }
